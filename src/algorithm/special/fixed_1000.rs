...
pub struct Fft1000<T: FftNum> {
    inner: Arc<dyn Fft<T>>,
    direction: FftDirection,
}

impl<T: FftNum> Fft1000<T> {
    pub fn new(direction: FftDirection) -> Self {
        let mut planner = FftPlannerScalar::<T>::new();
        let inner = planner.plan_fft(1000, direction);
        Self { inner, direction }
    }
}

impl<T: FftNum> Length for Fft1000<T> {
    fn len(&self) -> usize {
        1000
    }
}

impl<T: FftNum> Direction for Fft1000<T> {
    fn fft_direction(&self) -> FftDirection {
        self.direction
    }
}

impl<T: FftNum> Fft<T> for Fft1000<T> {
